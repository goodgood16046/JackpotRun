...
## S16 — 스핀 결과 패널: "얼마나·왜 얻었는지" (2026-08-01, Fable 설계)

사용자 피드백: "중간에 로그 형태가 아니라, **현재 스핀에서 점수를 얼마나 얻었고 뭐 때문에 얻었는지**가 있어야 하는데 없다."
현재는 한 줄 요약 + 로그 나열뿐이다. → **결과 패널(Gain Panel)** 로 교체한다.

### 데이터는 이미 엔진이 준다 (SpinResult, 표시 전용으로만 읽음)
`exp`(최종 EXP) · `score` · `coins` · `counts`(심볼별 개수) · `tagCounts` · `bestSetId`/`bestSetCount` ·
`skulls` · `jackpotSym` · `notes`(엔진 문구) · **`preMul`(전역배수 적용 전 EXP) · `mul`(적용 배율) · `flat`(가산분)**
→ 뷰 계층에서 이 값들과 `Symbols`/`Formulas` 표를 조합해 **기여 내역**을 만든다. 엔진 수정 금지.

### 레이아웃 (릴 아래, 액션바 위 — 기존 StageInfo+NotesFeed 자리를 대체)
1. **획득 대문짝** — `+33 EXP` 46pt w900 gold, 0.35s 카운트업 + gpop 팝인.
   그 아래 칩 2개: `점수 +9`(blue) · `코인 +1`(gold). 값이 0이면 칩 숨김.
2. **기여 내역 리스트**(최대 6줄, 각 줄: 아이콘 + 라벨 + 값):
   | 종류 | 예시 | 색 |
   |---|---|---|
   | 심볼 기본 | (체리 아이콘) `체리 ×3` `+12` | txt2 |
   | 세트 보너스 | `세트 3연속` `+18` | gold |
   | 특수 심볼 | `열쇠 획득` `+4` | teal |
   | 해골 페널티 | `해골 ×1` `−3` | red |
   | 가산 효과 | `증강 가산` `+5` | green |
   | 전역 배율 | `배율` `×1.45` | purple |
   계산: 심볼 기본 = `counts[sym] × Symbols.Exp(sym)`, 세트 = `preMul − 기본합 − flat + 페널티` 역산이 아니라
   **`Formulas`/`Symbols` 표에서 직접 산출**하고, 합이 `preMul`과 어긋나면 마지막 줄에 `기타 +N`으로 흡수(오차 은폐 금지).
   배율 줄은 `mul ≠ 1`일 때만, 값은 `×{NumberFormat.Fmt(mul)}`.
3. **세트 설명 박스**(웹 `.set-explain`) — 세트 성립 시에만: 보라~민트 그라데 배경 + 보라 테두리,
   `같은 심볼 {n}개 — 개수가 많을수록 보너스가 크게 늘어납니다` 22pt.
4. **로그는 축소** — 하단에 최근 3줄만, 18pt dim, 배경 없이(현재의 "로그 나열" 인상 제거).

### 연출
- 대문짝 숫자 카운트업(0.35s OutCubic) → 기여 내역 줄이 **0.05s 스태거로 위에서 아래로** 나타남(각 줄 페이드+8px 상승).
- 감산 줄은 붉은 플래시 1회, 배율 줄은 보라 글로우 펄스.
- 잭팟/4매치일 때 대문짝 숫자에 골드 글로우 강화 + 파티클(S15 B표) 동반.

### 규칙
- **엔진 값 재계산 금지** — 표시용 분해만 하고 최종 EXP/점수는 항상 `result.exp`/`result.score`를 그대로 쓴다.
- 내역 줄이 6개를 넘으면 작은 것부터 합쳐 `기타 +N`으로 축약.
- 스핀마다 갱신, 다음 스핀 시작 시 초기화(이전 값 잔상 금지).

## S15 — 파티클 기반 이펙트 전면 재작업 · 보상 선택 모달 · 해골 심볼 (2026-08-01, Fable 설계)

사용자 피드백 3건: ① 릴에 빈칸처럼 보이는 타일이 있다 ② 보상(증강) 선택 UI가 조악하다
③ **이펙트가 선만 긋는 수준이라 단조롭다 — 파티클로 제대로 만들어라.**

### A. 해골 심볼 (빈칸의 정체)
`sym_skull`은 어두운 배경(#2A0F14)만 굽고 도형을 그리지 않아 **빈 칸으로 보인다**(S8 결정 번복).
→ **해골 도형을 그린다**: 두개골(원+턱 사각) + 눈구멍 2개(배경색으로 파냄) + 코 삼각 + 이빨 3칸,
색 `#E8EAF2`. 저해상도에서도 읽히도록 굵게. 파일명은 그대로(`sym_skull`) — **재생성 시 overwrite:true 필요**.

### B. 이펙트 파티클 전면 재작업 (핵심)
현재 문제: Outline·플래시·단색 스윕 위주라 "선"처럼 보인다. **모든 주요 순간을 파티클 조합으로** 만든다.
공통 재료(신규 텍스처): `p_dot`(소프트 원, 기존 dot_soft 재사용 가능) · `p_star4`(4각 반짝) ·
`p_ring`(가는 링) · `p_shard`(마름모 파편) · `p_coin`(원+테두리) — 전부 가산합성.

| 순간 | 파티클 구성(레이어드) |
|---|---|
| 릴 정지 | ① 링 웨이브 1개(스케일 0.2→1.4, 0.25s, 알파 .7→0) ② 스파크 10개 방사(속도 300~600, 중력 200) ③ 먼지 6개 하단 |
| 세트 2매치 | 셀당 ① 심볼색 파편 12개 위로 분출 ② 링 1개 ③ 별 반짝 4개(랜덤 지연) |
| 세트 3매치 | 위 + ④ 셀 사이를 잇는 **트레일 파티클**(TrailModule, 0.3s) ⑤ 화면 하단에서 광입자 20개 상승 |
| 4매치 | 위 + ⑥ 각 셀 → 화면 중앙으로 **수렴 파티클** 30개(0.4s) ⑦ 중앙 도착 시 폭발 링 2연발 |
| 잭팟 | ⑧ 골드 코인 비 60개(중력, 회전, 1.6s) ⑨ 컨페티 80 ⑩ 방사 광선 회전 ⑪ 링 웨이브 3연발(0.1s 간격) ⑫ 화면 가장자리 상승 광입자 |
| 버튼 누름 | 링 임팩트 1 + 스파크 8(짧고 빠름, 0.25s) |
| 카드 선택 | 티어색 폭발 24 + 상승 입자 12 + 링 1 |
| EXP 획득 | 바 선두에서 흐르는 트레일 + 도착 시 작은 링 |
| 코인 획득 | `p_coin` 입자가 HUD로 포물선 비행(중력 + 회전), 도착 시 스파크 4 |
| 해골 페널티 | 검은 연기 8 + 붉은 파편 6 하강 |
| 배경(런 화면) | 아주 은은한 부유 광입자 3개/초(알파 .08) |

파티클 품질 규칙:
- **sizeOverLifetime**(커브: 0→1→0), **colorOverLifetime**(알파 페이드 + 색 그라데이션), `rotationOverLifetime`,
  `velocityOverLifetime`(x/y/z 같은 모드), 필요 시 `TrailModule`.
- 버스트는 **다단계**(예: 0s 12개 + 0.08s 6개)로 겹쳐 "터지는 맛"을 만든다.
- 모든 프리팹 `maxParticles` 명시, 동시 총량 상한 400.
- **Outline/플래시 단독 사용 금지** — 항상 파티클과 함께.

### C. 보상 선택 모달 (`PerkOfferPanel` 전면 재설계)
현재: 하단 시트 + 세로 목록 → **로그라이크 표준 형태로 교체**.
- **전체 화면 딤 0.72** + 중앙 정렬 모달(카드 3장 가로 배치).
- 헤더: 티어 배지(실버/골드/프리즘 색) + "증강 선택" 46pt + 부제(스테이지·티어 안내) 24pt.
- **카드 3장**: 각 320×620, gap 28. 구성(위→아래):
  아트 260 정사각(라운드, 티어색 테두리 3px + 티어색 글로우) → 이름 32 w900 → 티어 리본 →
  효과 설명 24 txt2(3줄) → 하단 [선택] 버튼(골드).
- **등장**: 딤 페이드 0.15s → 카드 3장 **0.08s 스태거 팝인**(scale .82→1.06→1, OutBack) + 각 카드 뒤 티어색 오라 파티클.
- **호버/누름**: 카드 -6px 부양 + 글로우 강화, 누름 시 scale .96.
- **선택 시**: 고른 카드 scale 1.12 + 티어색 폭발 파티클, 나머지 2장 0.15s 페이드아웃·축소, 0.35s 후 닫힘.
- 하단 보조 행: [보류](dev_holdfile 보유 시) · [재추첨 8코인](dev_retake 보유 시) — 고스트 버튼.
- 유물 노드도 같은 모달을 재사용(헤더 문구만 "유물 선택").

### D. PickView 토큰 통일 (S12c 잔여)
PickView가 아직 S10 팔레트를 쓴다 → S12 §0 토큰으로 교체(카드/칩/탭/요약 시트 전부).
