        ...
        private static bool AutoPlayDeep(RunController rc, int guardMax, long seed)
        {
            ...
            while (rc.State.Phase != RunPhase.GameOver && guard < guardMax)
            {
                ...
                switch (phase)
                {
                    ...
                    // 웹 파리티 P7-3(WEB_PARITY_DESIGN.md §1-A #19 3/4 슬라이스) — POUCH 2-step/REST·
                    // GAMBLE 심화 2택/3스테이지 연계 보너스도 전부 PickOffer(index) 단일 액션으로
                    // 라우팅된다(RunController.DispatchPickOffer). 항상 index 0 선택(스모크 목적).
